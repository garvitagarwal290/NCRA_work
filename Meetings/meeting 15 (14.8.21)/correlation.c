...
//the actual correlation loop. saves the correlation function in 'result' array
	float *shifted_g= (float*) malloc(sizeof(float)* num_pulses), *result=(float*) malloc(sizeof(float) * (2*num_pulses - 1));
	for (int i = -num_pulses+1; i < num_pulses; i++)
	{
		shift(g,shifted_g, i);
		result[i+num_pulses-1]=dot(f,shifted_g);
	}
