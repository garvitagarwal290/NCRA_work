...
//7th Jan 2022 ----------------------------------
//give definitio of variables used to define a custom cout
std::ofstream custom_cout::gbmon_log;
custom_cout::Tee custom_cout::tee( std::cout, custom_cout::gbmon_log );
custom_cout::TeeStream custom_cout::cout( custom_cout::tee );
//---------------------------------
...
Runtime::Runtime(Information info_,int nThreadMultiplicity_)
{
	...
	if(info.doReadFromFile)
	{
		...
		{
			custom_cout::cout<<endl<<endl<<"File contains "<<totalTime<<" seconds of data. Please give a starting time less than that"<<endl;
			exit(0);
		}
...
void Runtime::displayBlockIndex(int blockIndex)
{
	...
	if(info.doReadFromFile)
			custom_cout::cout<<'\r'<<"Block:"<<(blockIndex)-nActions*nThreadMultiplicity+1 <<" of "<<totalBlocks<<""<<std::flush;		
		/*else
			custom_cout::cout<<"\nPlot Window Block:"<<(blockIndex)-nActions*nThreadMultiplicity+1<<"\n\n"<<std::flush;
*/
}
...
void Runtime::intializeFiles()
{
	if(info.doWriteFiltered2D)
	{
		...
		{
			custom_cout::cout<<endl<<"ERROR: Cannot create outputfile. Possible permission issues?"<<endl;
			exit(1);
		}
	...
	/*while(!hasReachedEof)
	{
		...
		for(int i=0;i<nThreadMultiplicity;i++)
		{
			...
			{			
				...
				if(!keepRunning)
				{			
					custom_cout::cout<<endl<<"Terminating program.."<<endl;
					break;
				}
...
{
	custom_cout::cout<<endl<<"Closing pipe.."<<endl;
	...
	for(int k=0;k<nThreadMultiplicityTemp;k++)
	{
		...
		if(!info.doFilteringOnly)
		{
			...
			plot->show_status("Terminating GPTool..",13); //garvit 25 jan 2022
			timePlot+=omp_get_wtime();
		}
	}
	if(info.doPolarMode)
  	...
  	while(arg < argc)
  	{
    		if(argv[arg][0]=='-')
    		{
				...
				break;
        			...
        			case 'r':
        			{          
					std::cout<<"Reading from shm"<<endl;	
					...
					{   
						if(info.doReadFromFile)
						{
							info.startTime=info.stringToDouble(argv[arg+1]);
		  					if(info.startTime<0)
		  					{
		  						std::cout<<"Start time cannot be negetive!"<<endl;
		  						exit(0);
		  					}
		  				}
					}
				...
				case 'c':
				{
					...
					{
						info.scan_no= (int) info.stringToDouble(argv[arg+1]);
						arg+=2;
					}
				}
				break;
...
//.7th Jan 2022 Garvit Agarwal.---------------------------------- 	 	
	if(gbmon_logfile=="") 
	{
	std::cout<<endl<<"gbmon logfile name not provided. Exiting ..\n";
	exit(1);	
	}
	custom_cout::gbmon_log.open(gbmon_logfile.c_str(),std::ios_base::app);  	
//-----------------------------------------------------------

	if(!info.checkGptoolInputFileVersion())
	{
		custom_cout::cout<<"Old version of gptool.in file found.\n Replacing by new version formatting.\n Old version copied to gptool.in.oldver"<<endl;
		custom_cout::cout<<"Note this conversion will fail if the oldversion is not ver 1.5"<<endl;
	...
	if(gptoolPath==NULL) custom_cout::cout<<"Cannot plot summary pdf. Please set GPTOOL_PATH to path of plotgptoolsummary.py"<<endl;

	else
	{
		custom_cout::cout<<gptoolPath<<endl;
		custom_cout::cout<<"plotting summmary pdf..."<<endl;	
		stringstream plotcmd;
		plotcmd<<"python2 "<<gptoolPath<<"/plotgptoolsummary.py";	
		system(plotcmd.str().c_str());
	}*/

	runtime->plot->show_status("Terminated", 6); //garvit 25 jan 2022
