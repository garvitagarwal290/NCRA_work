...
////9,26jan, 20 feb 2022 garvit agarwal --------------------------------------

	string a=info.pgplot_devicetype,b=info.pgplot_devicetype, c;
	
	int pos=info.pgplot_devicetype.find("/png");
	if(pos == string::npos) pos = info.pgplot_devicetype.find("/PNG");
	if(pos != string::npos)
	{
		if( info.pgplot_devicetype.length() == 4)
		{
			a="main_plot.png"+info.pgplot_devicetype;
			b="info_plot.png"+info.pgplot_devicetype;
		}
		else
		{
			c=info.pgplot_devicetype.substr(0,pos);
			a=c+"/main_plot.png/png";
			b=c+"/info_plot.png/png";
		}
	}

	main_plot= cpgopen(a.c_str());	
	cpgpap (info.plotwindow_width,0.618); 
	cpgpage();
	cpgsch(0.85);

	info_plot= cpgopen(b.c_str());
	cpgpap (3.5,info.plotwindow_width*0.618/3.5); //9.0
	cpgpage();
	cpgsch(2.3);
	...
	stringstream line;
	line.str("");
	line<<"Starting GPTool..";
	cpgtext(0,80,line.str().c_str());
	line.str("");
	line<<"Loading Plots..";
	cpgtext(0,65,line.str().c_str());
	

	cpgslct(main_plot);
//9,26 jan 2022 garvit agarwal --------------------------------------

}

//9 Jan 2022 Garvit_______________________
void Plot::plotInfo(int index)
{
	//cpgslct(plot2);
	cpgask(0);
	cpgpage();
	...
	tm *st= localtime(&info.start_time);
	line.str("");	
	line<<"Start time: "<<st->tm_hour<<":"<<st->tm_min<<":"<<st->tm_sec<<"  "<<st->tm_mday<<":"<<st->tm_mon+1<<":"<<st->tm_year+1900;
	cpgtext(0,90,line.str().c_str());

	double elapsed=difftime(time(0),info.start_time);
	int hours, min, sec;
	hours=elapsed/3600;
	min=(elapsed/60) - hours*60;
	sec=elapsed - (min*60) - (hours*3600);
	
	line.str("");	
	line<<"Elapsed time: "<<hours<<":"<<min<<":"<<sec;
	cpgtext(0,83,line.str().c_str());

	line.str("");
	line<<"Plot Frame number: "<<index+1;
	cpgtext(0,76,line.str().c_str());

	line.str("");
	line<<"Source: "<<info.pulsarName;
	cpgtext(0,69,line.str().c_str());

	line.str("");
	line<<"Period: "<<setprecision(12)<<info.periodInMs<<" ms";
	cpgtext(0,62,line.str().c_str());

	line.str("");
	line<<"DM: "<<setprecision(10)<<info.dispersionMeasure<<" pc cm^-3";
	cpgtext(0,55,line.str().c_str());

	line.str("");	
	line<<"Lowest frequency: "<<info.lowestFrequency<<" MHz";
	cpgtext(0,48,line.str().c_str());

	line.str("");	
	line<<"Bandwidth: "<<info.bandwidth<<" MHz";
	cpgtext(0,41,line.str().c_str());

	line.str("");	
	line<<"Number of channels: "<<info.noOfChannels;
	cpgtext(0,34,line.str().c_str());

	line.str("");	
	line<<"Sampling interval: "<<info.samplingInterval*1000.0<<" ms";
	cpgtext(0,27,line.str().c_str());

	line.str("");	
	line<<"Beam Type: "<<info.modeOperation;
	cpgtext(0,20,line.str().c_str());
...
void Plot::show_status(char* text, int pos)
{
	cpgslct(info_plot);
	cpgask(0);
	cpgsch(2.3);
...
//----------------------------------------
//added by Garvit Agarwal ~Dec2021-Jan2022, 26 jan for proper waterfall plot in /PNG device
	
	int pos=info.pgplot_devicetype.find("/png");
	if(pos != string::npos) pos = info.pgplot_devicetype.find("/PNG");
	if(pos == string::npos)
	{
		cpgscir(16,99);	//these 3 lines are required
		cpgscr(16,0.0,0.0,0.0);
		cpgscr(99,1.0,1.0,1.0);	
	}

	//int a,b;  //these 3 lines are just to find out the range of color indices being used
	//cpgqcir(&a,&b);
	//printf("\n%d %d\n",a,b);
...
void Plot::plotBlockIndex(int index)
{
	...
	if(info.doReadFromFile)
		text1<<"Block: "<<info.startBlockIndex+index+1<<"/"<<totalBlocks<<" | Time: "<<setiosflags(ios::fixed) << setprecision(1)<<(info.startBlockIndex+index)*info.blockSizeSec<<"/"<<setiosflags(ios::fixed) << setprecision(1)<<(totalBlocks-1)*info.blockSizeSec<<" sec | Scan No: "<< info.scan_no;
	else
	{
		if(info.scan_no == 0) text1<<"Block: "<<index+1<<" | Time: "<<setiosflags(ios::fixed) << setprecision(1)<<(index+1)*info.blockSizeSec<<" sec";
		else text1<<"Block: "<<index+1<<" | Time: "<<setiosflags(ios::fixed) << setprecision(1)<<(index+1)*info.blockSizeSec<<" sec | Scan No: "<< info.scan_no;
	}
...
void Plot::plotTitle()
{
	...
	cpgswin(0,100,0,100);
	ostringstream text1;
	text1<<"GMRT Pulsar Tool ver 4.3.6_gbmon";
	cpgtext(0,80,text1.str().c_str());
	cpgsci(1);
...
{
	cpgslct(info_plot);
	plotInfo(index); //garvit 9 jan 2022
	cpgslct(main_plot);
	
	//Garvit Agarwal 17 jan 2022 shifted from Plot()-------------------
	if(info.doManualMode)
		cpgask(1);
	else
		cpgask(0);
