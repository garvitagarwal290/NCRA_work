...
short int Information::checkGptoolInputFileVersion()
{
	...
	if(!filepar.is_open())
	{
		custom_cout::cout<<"gptool.in not found!"<<endl;
	 	custom_cout::cout<<"A sample gptool.in file has been written to the current directory."<<endl;
		writeWpmonIn();
	  	exit(0);
	}
...
void Information::readGptoolInputFile()
{
	  ...
	  while(getline(filepar,line))
	  {
		...
		switch(k)
			...
			case 62:
				break;
			//garvit agarwal 15 January 2022--------------------
			case 63:
			{
				if(float(stringToDouble(tempStr))==0.0) break;
				plotwindow_width=float(stringToDouble(tempStr));
				break;
			}
			case 64:
			{
				pgplot_devicetype=tempStr;
				int pos=pgplot_devicetype.find("/png");
				if(pos == string::npos) pos = pgplot_devicetype.find("/PNG");
				if(pos == string::npos) 
				{
					if(pgplot_devicetype != "/XS" && pgplot_devicetype != "/xs") pgplot_devicetype="/xs";
				}
	...
	{
		custom_cout::cout<<"Error in line of gptool.in"<<endl;
		custom_cout::cout<<"Channel filtering cannot be done after time filtering if data is normalized."<<endl;
		custom_cout::cout<<"Recommended option is to do channel filtering first."<<endl;
		exit(0);
	}
...
void Information::parseManFlagList(std::string& s)
{
	...
	while(p<slen)
	{
		if(i>=nBadChanBlocks)
		{
			custom_cout::cout<<"Error in line 60 of gptool.in"<<endl;
			custom_cout::cout<<"Expected exactly "<<nBadChanBlocks<<" list of bad sub-bands"<<endl;
			exit(0);
		}
		if(s[p]=='[')
			p1=p;
		...
		{
			custom_cout::cout<<"Error in line 60 of gptool.in"<<endl;
			custom_cout::cout<<"Expected [ to mark the start of a bad sub-band"<<endl;
			exit(0);
		}
		...
		{
			custom_cout::cout<<"Error in line 60 of gptool.in"<<endl;
			custom_cout::cout<<"Expected ,"<<endl;
			exit(0);
		}
		while(s[++p]!=']' && p<slen);
		if(s[p]==']')
		...
		{
			custom_cout::cout<<"Error in line 60 of gptool.in"<<endl;
			custom_cout::cout<<"Expected , between list of sub-bands"<<endl;
			exit(0);
		}
		p++;
...
void Information::errorChecks()
{
	...
	if(doReadFromFile==1)
	{
		...
		if(!testExistance.is_open())
  		{
    			custom_cout::cout<<"No file with name: "<< filepath<<endl;
    			erFlag=1;
 		}
	}
	if(doPolarMode != 0 && doPolarMode != 1)
	{
		custom_cout::cout<<"Error in line 5 of gptool.in:"<<endl<<"Polarization mode must be either 0 for for Intensity or 1 for full stokes"<<endl;
		erFlag=1;
	}
	if(doPolarMode && polarChanToDisplay>3)
	{
		custom_cout::cout<<"Error in line 28 of gptool.in:"<<endl;
		custom_cout::cout<<"Polarization channel to display must be between 0 and 3 OR -1 to display all four channels"<<endl;
		erFlag=1;
	}
	if(sampleSizeBytes !=1 && sampleSizeBytes !=2 && sampleSizeBytes != 4)
	{
		custom_cout::cout<<"Error in line 6 of gptool.in:"<<endl<<"The tool can only process 1 or 2 byte integer or 4 byte float."<<endl;
		erFlag=1;
	}
	if(sidebandFlag !=0 && sidebandFlag != 1)
	{
		custom_cout::cout<<"Error in line 11 of gptool.in:"<<endl<<"-1 for decreasing channel ordering, +1 for increasing."<<endl;
		erFlag=1;
	}
	...
	{
		custom_cout::cout<<"Error in 21 of gptool.in:"<<endl<<"Number of bins in folded profile must be positive definite."<<endl;
		erFlag=1;
	}
	if(profileOffset >1 || profileOffset<0)
	{
		custom_cout::cout<<"Error in 22 of gptool.in:"<<endl<<"Pulsar phase offset must be between 0 and 1"<<endl;
		erFlag=1;
	}
	if(doManualMode !=0 && doManualMode !=1)
	{
		custom_cout::cout<<"Error in line 30 of  gptool.in:"<<endl<<"0 for automatic update and 1 for manual update."<<endl;
		erFlag=1;
	}
	if(startChannel>noOfChannels)
	{
		custom_cout::cout<<"Error in line 34 of  gptool.in:"<<endl<<"Channels to skip cannot be greater than number of channels"<<endl;
		erFlag=1;
	}
	
	if(stopChannel<startChannel)
	{
		custom_cout::cout<<"Error in line 35 of  gptool.in:"<<endl<<"The sum of channels to skip in the beginning and end exceeds the number of channels"<<endl;
		erFlag=1;
	}
	...
	{
		custom_cout::cout<<"Error in line 36 of  gptool.in:"<<endl<<"0 for no frequency domain filtering and 1 for filtering on."<<endl;
		erFlag=1;
	}
	
	if(bandshapeToUse !=2 && bandshapeToUse !=3 && bandshapeToUse !=4)
	{
		custom_cout::cout<<"Error in line 37 of  gptool.in:"<<endl<<"1 for normalized bandshape; 2 for mean to rms bandshape and 3 for both."<<endl;
		erFlag=1;
	}
	
	if(chanCutOffToRMS <=0)
	{
		custom_cout::cout<<"Error in line 38 of  gptool.in:"<<endl<<"Cutoff must be positive."<<endl;
		erFlag=1;
	}
	
	if(doWriteChanFlags!=0 && doWriteChanFlags!=1)
	{
		custom_cout::cout<<"Error in line 52 of  gptool.in:"<<endl<<"1 to write channel flags, 0 otherwise."<<endl;
		erFlag=1;
	}
	
	if(doTimeFlag !=0 && doTimeFlag !=1)
	{
		custom_cout::cout<<"Error in line 41 of  gptool.in:"<<endl<<"0 for no time domain filtering and 1 for filtering on."<<endl;
		erFlag=1;
	}
	...
	{
		custom_cout::cout<<"Error in line 42 of  gptool.in:"<<endl<<"1 to normalize data, 0 otherwise."<<endl;
		erFlag=1;
	}
	
	if(timeFlagAlgo!=2 && timeFlagAlgo!=1)
	{
		custom_cout::cout<<"Error in line 43 of  gptool.in:"<<endl<<"1 to use histogram based, 2 to use MAD basesd."<<endl;
		erFlag=1;
	}
	
	if(timeCutOffToRMS <=0)
	{
		custom_cout::cout<<"Error in line 44 of  gptool.in:"<<endl<<"Cutoff must be positive."<<endl;
		erFlag=1;
	}
	
	if(doWriteTimeFlags!=0 && doWriteTimeFlags!=1)
	{
		custom_cout::cout<<"Error in line 53 of  gptool.in:"<<endl<<"1 to write time flags, 0 otherwise."<<endl;
		erFlag=1;
	}
	...
	{
		custom_cout::cout<<"Error in line 47 of  gptool.in:"<<endl<<"Smooth window length must be positive and less than the number of channels."<<endl;
		erFlag=1;
	}
	
	if(normalizationProcedure!=2 && normalizationProcedure!=1)
	{
		custom_cout::cout<<"Error in line 48 of  gptool.in:"<<endl<<"1 to use cumulative smooth bandshape, 2 to use externally supplied bandshape.dat."<<endl;
		erFlag=1;
	}
	if(doReplaceByMean!=0 && doReplaceByMean!=1 &&  doReplaceByMean!=2)
	{
		custom_cout::cout<<"Error in line 49 of  gptool.in:"<<endl<<"0 to replace by zeros, 1 to replace flagged points by modal (median) value, 2 to replace by smooth bandshape"<<endl;
		erFlag=1;
	}
	if(doReplaceByMean==1 && doUseNormalizedData!=1)
	{
		custom_cout::cout<<"Invalid combination of choices in line 42 and 49."<<endl<<"Data must be normalized to replace by modal (median) values.";
		erFlag=1;
	}
	if(doReplaceByMean==1 && doTimeFlag!=1)
	{
		custom_cout::cout<<"Invalid combination of choices in line 41 and 49."<<endl<<"Time filtering must be on to replace by modal (median) values.";
		erFlag=1;
	}
	if(normalizationProcedure==2)
	{
		...
		if(!testExistance.is_open())
  		{
    			custom_cout::cout<<"No bandshape.dat file found."<<endl;
    			erFlag=1;
 		}
	}
	if(doWriteFullDM!=0 && doWriteFullDM!=1)
	{
		custom_cout::cout<<"Error in line 55 of  gptool.in:"<<endl<<"1 to write dedispersed time series, 0 otherwise."<<endl;
		erFlag=1;
	}
	if(doWriteFiltered2D!=0 && doWriteFiltered2D!=1)
	{
		custom_cout::cout<<"Error in line 54 of  gptool.in:"<<endl<<"1 to write 2D time-frequency data, 0 otherwise."<<endl;
		erFlag=1;
	}
	if(nBadChanBlocks!=0)
	{
		for(int i=0;i<nBadChanBlocks;i++)	
		{
			...
			{
				custom_cout::cout<<"Error in line 60 of gptool.in"<<endl;
				custom_cout::cout<<"In sub-band "<<i+1<<": channel number cannot be less than zero."<<endl;
				erFlag=1;
			}
			if(badChanBlocks[i*2]>noOfChannels || badChanBlocks[i*2+1]>noOfChannels )
			{
				custom_cout::cout<<"Error in line 60 of gptool.in"<<endl;
				custom_cout::cout<<"In sub-band "<<i+1<<": channel number cannot be greater than the number of channels."<<endl;
				erFlag=1;
			}
			if(badChanBlocks[i*2]>=badChanBlocks[i*2+1])
			{
				custom_cout::cout<<"Error in line 60 of gptool.in"<<endl;
				custom_cout::cout<<"In sub-band "<<i+1<<": end channel must be strictly greater than start channel."<<endl;
				erFlag=1;
			}
		}			
...
void Information::genMJDObs()
{
	...
	ostringstream  headerName;
  	if(doReadFromFile==1) //Read from file
    	...
    	{
      		custom_cout::cout<<headerName<<":header file not found!"<<endl;
      		exit(1);
    	}
    	...
    	if(YYYY < 0 || MM < 1 || MM > 12 || DD < 1 || DD > 31 || HH < 0 || HH > 23 || mm < 0 || mm > 59 || sec < 0.0 || sec >= 60.0 )
    	{
      		...
      		custom_cout::cout<<"\t'raw.hdr' file should STRICTLY contain the IST time and IST date of Observation in this format\n";
      		custom_cout::cout<<"#Start time and date\n";
      		custom_cout::cout<<"IST Time: HH:mm:SS.ssss\n";
      		custom_cout::cout<<"Date: DD:MM:YYYY\n\n";
      		exit(1);
    	}
...
void Information::getPsrcatdbPath()
{
	...
	{	
		...
		if((int)eof==0)
		{
			custom_cout::cout<<"No psrcat.db found!"<<endl;
			exit(0);
		}
	}
...
void Information::checkPulsarName()
{
	//Generate JName and check if source name is valid
    	...
    	{
        	custom_cout::cout<<"Error in line 14 of gptool.in:"<<endl;
        	custom_cout::cout<<"Incorrect pulsar name."<<endl;
        	exit(1);
    	}
...
void Information::display()
{
	...
	displays<<endl<<"gptool v 4.3.6_gbmon"<<endl;
	if(doFilteringOnly)
	...
	if(!doFilteringOnly)
	{
		...
		displays<<endl<<"Source: "<<pulsarName<<endl;
		
		displays<<"Period: "<<setprecision(12)<<periodInMs<<" ms"<<endl;
		displays<<"Dispersion measure: "<<setprecision(10)<<dispersionMeasure<<" pc/cc"<<endl;	
	
	
	
		/*if(refFrequency)
			custom_cout::cout<<endl<<"Dedispersion will be done with respect to the highest frequency."<<endl;
		else
			custom_cout::cout<<"Dedispersion will be done with respect to the lowest frequency."<<endl; */
	...
	if(doChanFlag)
	{
		...
		/*
		if(chanFlagAlgo==1)
			custom_cout::cout<<"\tHistogram based algorithm selected."<<endl;		
		else if(chanFlagAlgo==2)
			custom_cout::cout<<"\tMAD based algorithm selected."<<endl;	
		*/	
	...
	{
		if(flagOrder==1)
			custom_cout::cout<<endl<<"Channel filtering will be done first."<<endl;
		else if(flagOrder==2)
			custom_cout::cout<<"Time filtering will be done first."<<endl;
		else if(flagOrder==3)
			custom_cout::cout<<"Time & channel filtering will be done independent of each other."<<endl;
		custom_cout::cout<<endl;
	}*/
	...
	custom_cout::cout<<displays.str().c_str();
	start_time = time(0);   
...
{
	custom_cout::cout<<"gptool -f [filename] -r -shmID [shm_ID] -s [start_time_in_sec] -o [output_2d_filtered_file] -m [mean_value_of_2d_op] -tempo2 -nodedisp  -zsub -inline -gfilt"<<endl<<endl;
	custom_cout::cout<<"-f [filename] \t\t\t :Read from GMRT format file [filename]"<<endl;
	custom_cout::cout<<"-r  \t\t\t\t :Attach to shared memory"<<endl;
	custom_cout::cout<<"-shmID [shm_ID] \t\t :shm_ID = \t1 -> Standard correlator shm \n\t\t\t\t\t\t2-> File simulator shm \n\t\t\t\t\t\t3-> Inline gptool shm"<<endl; 
	custom_cout::cout<<"-s [start_time_in_sec] \t\t :start processing the file after skipping some time"<<endl;
	custom_cout::cout<<"-o [output_2d_filtered_file] \t :path to GMRT format filtered output file"<<endl;
	custom_cout::cout<<"-m [mean_value_of_2d_op] \t :mean value of output filtered file \n \t\t\t\t this is the value by which the gptool normalized data \n\t\t\t\t is scaled by before writing out filtered file"<<endl;
	custom_cout::cout<<"-tempo2 \t\t\t :use tempo2 instead of tempo1 to get polycos"<<endl;
	custom_cout::cout<<"-nodedisp \t\t\t :just perform RFI filtering without further processing"<<endl;
	custom_cout::cout<<"-zsub \t\t\t\t :optimized zero DM subtraction. \n\t\t\t\t Experimental feature, use with caution"<<endl;
	custom_cout::cout<<"-inline \t\t\t :inline mode of gptool, read from a shared memory \n\t\t\t\t write filtered output to another shared memory"<<endl;
	custom_cout::cout<<"-gfilt \t\t\t\t :turns off all filtering options, \n\t\t\t\t will over-ride gptool.in inputs"<<endl;
